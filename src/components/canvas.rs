...
use web_sys::{HtmlCanvasElement, MouseEvent, window, HtmlBodyElement};
...
pub fn canvas(props: &CanvasProps) -> Html {
    ...
    // Main rendering effect - separate from animation
    {
        ...
        use_effect_with(
            (rover_state.clone(), cell_size_val, traveled_path.clone(), amber_dobs.clone()),
            move |_| {
                let render = move || {
                    ...
                    if let Some(canvas) = canvas_ref.cast::<HtmlCanvasElement>() {
                        ...
                        if !traveled_path.is_empty() {
                            ...
                            for &(x, y) in traveled_path.iter().skip(1) {
                                ...
                                context.begin_path();
                                context.arc(px, py, 3.0, 0.0, std::f64::consts::PI * 2.0).unwrap();
                                context.fill();
                            }
                        }
                        ...
                        if !rover_state.path.is_empty() && rover_state.path.len() > 1 {
                            ...
                            if rover_state.path.len() > 2 {
                                for &(x, y) in rover_state.path[1..rover_state.path.len()-1].iter() {
                                    let px = (x as f64) * cell_size + (cell_size / 2.0);
                                    let py = (y as f64) * cell_size + (cell_size / 2.0);
                        ...
                        if start_x < width && start_y < height {
                            ...
                            context.fill_text("S", x + cell_size / 2.0, y + cell_size / 2.0 + 4.0).unwrap();
                        }
                        ...
                        if goal_x < width && goal_y < height {
                            ...
                            context.set_font("bold 11px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif");
                            context.set_text_align("center");
                            context.fill_text("G", x + cell_size / 2.0, y + cell_size / 2.0 + 4.0).unwrap();
                        }
                        ...
                        if rx < width && ry < height {
                            ...
                            context.set_stroke_style(&JsValue::from_str(&format!("rgba(251, 146, 60, {})", pulse * 0.8)));
                            context.set_line_width(3.0);
                            context.begin_path();
                            context.arc(cx, cy, 2.0 * cell_size, 0.0, std::f64::consts::PI * 2.0).unwrap();
                            context.stroke();
                            
                            // Inner circle for visual effect
                            context.set_stroke_style(&JsValue::from_str(&format!("rgba(251, 146, 60, {})", pulse * 0.5)));
                            context.set_line_width(2.0);
                            context.begin_path();
                            context.arc(cx, cy, 1.5 * cell_size, 0.0, std::f64::consts::PI * 2.0).unwrap();
                            context.stroke();
                            ...
                            context.begin_path();
                            context.arc(cx, cy, 10.0, 0.0, std::f64::consts::PI * 2.0).unwrap();
                            context.fill();
                            
                            // Inner circle
                            context.set_fill_style(&JsValue::from_str("#a0926b"));
                            context.begin_path();
                            context.arc(cx, cy, 6.0, 0.0, std::f64::consts::PI * 2.0).unwrap();
                            context.fill();
                            
                            // Inner highlight
                            context.set_fill_style(&JsValue::from_str("rgba(255, 255, 255, 0.4)"));
                            context.begin_path();
                            context.arc(cx - 2.0, cy - 2.0, 2.0, 0.0, std::f64::consts::PI * 2.0).unwrap();
                            context.fill();
                        }
                    }
                };
