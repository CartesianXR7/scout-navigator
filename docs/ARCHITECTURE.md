...
| 3 · Rover & Path                   | Keeps rover pose, travelled path… | • Program state • Start/goal nodes + FOM (L-4) • Algorithm impl                | • Pose + paths → L-2 • Status → backend              |
| 4 · Fixed Obstacle Map (FOM)       | Authoritative obstacle list…      | • Initial grid • Converted DOBs (L-1) • Rover requests                         | • SOB list → L-3 • Original SOB list → L-2, L-1      |
