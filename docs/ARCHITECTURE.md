...
| ---------------------------------- | --------------------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------- |
| 1 · Dynamic Obstacle Map (DOB)     | Manages user-placed obstacles…    | • Mouse clicks (UI) • Rover coordinates (L-3) • Initial SOB layout (L-4)       | • New SOB coords → L-4 • Amber/blue updates → L-2 |
| 2 · Grid UI / Visualisation        | Single source of truth…           | Inputs from L-1, L-3, L-4                                                      | Draw commands → browser                              |
| 3 · Rover & Path                   | Keeps rover pose, travelled path… | • Program state<br>• Start/goal nodes + FOM (L-4) • Algorithm impl             | • Pose + paths → L-2 • Status → backend              |
| 4 · Fixed Obstacle Map (FOM)       | Authoritative obstacle list…      | • Initial grid<br>• Converted DOBs (L-1) • Rover requests                      | • SOB list → L-3 • Original SOB list → L-2, L-1      |
